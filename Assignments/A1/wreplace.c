...
bool substring_found(enum MODES mode, char* str, char* word){
 ...
 // Logic for RC 
 // function encodes all words (transforming word to ****), then return the resulting string dynamically
char *redact_word(char *before, char *word, int *pt_swap_count){
  ...
  while (i < strlen(before))
  {
    ...
    if (strstr(&before[i], word) == &before[i] ){
      ...
      if(pt_swap_count != NULL) *pt_swap_count +=1; //increment swap count
    }
 ...
 // Logic for RI. 
 // function encodes all words (transforming word to ****) while ignoring case, then return the resulting string dynamically
char *redact_word_ignorecase(char *before, char *word, int *pt_swap_count){
 ...
 // Logic for UK. 
 // function replaces all encoded words (i.e ***) with the given word, then return the resulting string dynamically
char *decode_word_keep_case(char *before, char *word,  int *pt_swap_count){
 ...
 // Logic for UM. 
 // function replaces all encoded words (i.e ***) with the given word, then return the resulting string dynamically.
 // the word replaced has the same case as the letter before, if none then the same case as the letter after, if none then the the same case as the word provided.
 // EX: These ***S are so funny!
