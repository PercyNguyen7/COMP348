...
//get_response processes user's response and return a number corresponding to an action
...
void display_summary(bool match_found, int total_swap_count, int total_line_changed, enum EXIT_TYPE exit_type ){
    ...
    if(match_found){
        if (exit_type == NORMAL_E){
            printf("Changed %d word(s) in %d line(s). Exiting normally and returning code 0.\n\n", total_swap_count,total_line_changed);
        } else if (exit_type == QUIT_E) {
            printf("Changed %d word(s) in %d line(s). Exiting normally and returning code 1.\n\n", total_swap_count,total_line_changed);
        }
    }
    else{printf("No match found. Exiting normally and returning code 2.\n\n");
    }
